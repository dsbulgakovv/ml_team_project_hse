...
def create_user_df(encoder, age: str, income: str, sex: str, kids_flg: int) -> pd.DataFrame:
...
def data_find_best_content(
    ...
    if (genre is None) & (content_type is None): # Если жанр и тип контента пропуски
        ...
        best_films_for_user = data_user_to_user_model[
            (data_user_to_user_model['7_clusters']==predict_cluster) & (data_user_to_user_model['content_type']==f'{content_type}')
        ]['item_id'].value_counts()
    elif (genre is not None) & (content_type is None): # Если жанр заполнен и тип контента пропуск
        best_films_for_user = data_user_to_user_model[
            (data_user_to_user_model['7_clusters']==predict_cluster) & (data_user_to_user_model[genre]==1)
        ]['item_id'].value_counts() 
    else: # Если все заполнено
        best_films_for_user = data_user_to_user_model[
            (data_user_to_user_model['7_clusters']==predict_cluster) & (data_user_to_user_model[genre]==1) & (data_user_to_user_model['content_type']==f'{content_type}')
        ]['item_id'].value_counts()
...
def show_recommendations(best_films_for_user:pd.Series, items_data:pd.DataFrame):
    ...
    while input_val == 'next':
        ...
        df_to_show = items_data.merge(df_to_show_index, how='inner', on='item_id') # тут ходим в табличку items за описанием по 10 фильмам
